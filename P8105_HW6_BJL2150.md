...
validation\_df = crossv\_mc(birthweight\_clean, 100)

validation\_df = validation\_df %&gt;% mutate(bw\_linear\_one = map(train, ~lm(bwt ~ momage + mrace + smoken, data = .x)), length\_ges\_linear = map(train, ~lm(bwt ~ blength + gaweeks, data = .x)), linear\_interaction = map(train, ~lm(bwt ~ bhead + blength + babysex + bhead*blength*babysex, daya = .x)))
