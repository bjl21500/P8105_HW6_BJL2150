...
# This is the working dataframe where the 4 cities were filtered out successfully.
...
hom\_unsolved = sum(resolution == "unsolved")) \#\# Using the 'glm' function

baltimore = homicide\_citystate %&gt;% filter(city\_state == "Baltimore, MD")

logistic\_homicide = glm(solved ~ victim\_age + race + victim\_sex, data = baltimore, family = binomial()) %&gt;% broom::tidy() %&gt;% prop.test(x = ) mutate(new = map2(.x = estimate, .y = , ~prop.test(x = .x, n = .y)), tidy\_tests = map(prop\_tests, broom::tidy)) \`\`\`
