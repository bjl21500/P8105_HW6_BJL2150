...
output &lt;- df %&gt;% select(-am) %&gt;% names() %&gt;% paste('am~',.) %&gt;% map\_df(~tidy(glm(as.formula(.x), data= df, family = "binomial"), conf.int=TRUE, exponentiate=TRUE))

manhattan\_nest\_lm\_res = manhattan\_airbnb %&gt;% group\_by(neighborhood) %&gt;% nest() %&gt;% mutate(models = map(data, ~lm(price ~ stars + room\_type, data = .x)), models = map(models, broom::tidy)) %&gt;% select(-data) %&gt;% unnest()
