...
In the code chunk below, the new variable 'city_state' was created. Also created
was a binary variable for disposition which was named 'solved'. In this variable,
1 = "Closed by arrest" and 0 = "Open/ no arrest" and "Closed without arrest".
...
```
  ...
  Figure 2 illustrates the estimated odds ratios and confidence intervals per city.
...
factors for newborn's birthweight. Some of the variables include gestational age 
in weeks, baby's sex, baby's head circumference, baby's birth weight, family 
monthly income, mother's age at delivery and average number of cigarettes smoked 
per day during pregnancy. This dataset will be used in Problem 2.
...
birthweight. From this, I chose to include three of the four risk factors, which 
are the aformentioned predictors. In running a linear regression model, there was 
an observed statistical significance for all of the predictors. Therefore, they remained within my hypothesized linear model.
...
added to my proposed model are mother's age at delivery, mother's race, and average
number of cigarettes smoked per day during pregnancy. Due to the high root mean 
square value, the conclusion can be made that this model inappropriate.
