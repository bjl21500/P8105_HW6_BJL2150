...
homicide_citystate = homicide_data %>%
  ...
  select(city_state, victim_race, solved, victim_age, victim_sex) %>%
...
```{r}
logistic_homicide = glm(solved ~ victim_age + race + victim_sex, data = homicide_citystate, family = binomial()) %>%
  broom::tidy()
```
